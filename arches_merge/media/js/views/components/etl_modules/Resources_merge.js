...
import baseStringEditorTemplate from "templates/views/components/etl_modules/Resources_merge.htm";
...
const ViewModel = function (params) {
    ...
    const normalizeSummaryEntries = (entries) => {
        ...
        return entries.map((entry) => {
            const label =
                entry?.label || (typeof entry === "string" ? entry : "");
    ...
    this.changeSummary = {
        ...
        changed: ko.observableArray(
            normalizeSummaryEntries(changeSummary.changed),
        ),
        deleted: ko.observableArray(
            normalizeSummaryEntries(changeSummary.deleted),
        ),
        mergeCounts: ko.observableArray(changeSummary.mergeCounts || []),
        sourceReferenceCounts: ko.observableArray(
            changeSummary.sourceReferenceCounts || [],
        ),
    };
    ...
    this.resourceBaseDisplay = ko.pureComputed(() =>
        self.getResourceDisplay(self.resourceBase()),
    );
    this.loadDetailsBaseDisplay = ko.pureComputed(() =>
        self.getResourceDisplay(self.load_details?.baseResource),
    );
    this.loadDetailsMergeDisplay = ko.pureComputed(() => {
        ...
        const ids = Array.isArray(mergeResources)
            ? mergeResources
            : [mergeResources];
        const formatted = ids
            .filter(Boolean)
            .map((id) => self.getResourceDisplay(id));
        return formatted.join(", ");
    });
    if (this.load_details?.baseResource) {
        this.fetchResourceDisplay(this.load_details.baseResource);
    }
    const initialMergeResources = Array.isArray(
        ...
        const keys = Array.from(keySet).sort((a, b) => {
            const parseKey = (key) => {
                const match = key.match(
                    /^(.*)_(pre|added|deleted|excluded|post|post_merge)$/,
                );
        ...
        setTimeout(() => {
            ...
            if (filterText === "") {
                    ...
                    return (
                        ...
                        (tile.tiledata ?? []).some(
                            (td) =>
                                td &&
                                td.some((entry) =>
                                    entry.toLowerCase().includes(filterText),
                                ),
                        )
                    );
                };
                ...
                const filterNodegroupAttrs = (ng) => {
                    return (
                        ng.name.toLowerCase().includes(filterText) ||
                ...
                const filtered =
                    this.mergeableNodeGroups().filter(filterNodegroupAttrs);
    ...
    this.addResource = () => {
        ...
        if (resourceId && this.isValidUuid(resourceId)) {
            ...
            if (
                !this.mergeResources().includes(resourceId) &&
                baseId !== resourceId
            ) {
    ...
    const normalizeMergeableGroups = (groups) => {
        ...
        return groups.map((group) => {
            const tiles = Array.isArray(group.tiles)
                ? group.tiles.map((tile) => {
                      ...
                      return {
                          ...
                          resourceId: resourceId
                              ? self.resourceDisplayMap()[resourceId]
                              : undefined,
                          status,
                          branch_name: tile?.branch_name || "",
    ...
    this.downloadPreviewCSV = function () {
        ...
        csv +=
            [escapeCSV("User"), escapeCSV(self.userName() || "—")].join(",") +
            "\n";
        ...
        csv +=
            [escapeCSV("Merge Resource(s)"), escapeCSV(mergeList)].join(",") +
            "\n";
        ...
        previewRows.forEach((r) => {
            const nodegroupExcluded = self.isNodegroupExcluded(r.nodegroupId)
                ? "Yes"
                : "No";
        ...
        */
            const tileDataCombined =
                `${r.tiledataLabel || ""}: ${r.tiledataValue || ""}`.trim();
            csv +=
                [
    ...
    this.recalcButtonLabel = ko.pureComputed(function () {
        ...
        return self.exclusionsDirty()
            ? "Recalculate Dict Counts"
            : "Dict Counts Up to date";
    });
    ...
    this.toggleExcludeTile = function (tileId) {
        ...
        if (blocked) {
            console.log(
                "Cannot exclude tile because ancestor is excluded:",
                ancestors,
            );
            return;
        }
        ...
        console.log(
            "Updated excludedTileIds:",
            Array.from(self.excludedTileIds()),
        );
    ...
    this.toggleExcludeNodegroup = function (nodegroupId) {
        ...
        for (let t of group.tiles) {
            ...
            if (blocked) {
                console.log(
                    "Cannot exclude nodegroup because ancestor is excluded for tile:",
                    t.tileId,
                );
                return;
            }
    ...
    this.applyExclusions = function (groups) {
        ...
        return groups
            .map((group) => {
                const filteredTiles = (group.tiles || []).filter(
                    (tile) => tile.tileId && !excludedSet.has(tile.tileId),
                );
        ...
        groups.forEach((g, groupIndex) => {
            ...
            tiles.forEach((t) => {
                ...
                tdEntries.forEach((entry) => {
                    ...
                    const tileKey =
                        (g.nodegroupId || "") + "::" + (t?.tileId || "NO_TILE");
    ...
    self.pagedRows = ko.computed(function () {
        ...
        slice.forEach((r) => {
            r.showGroupCell = false;
            r.rowspan = 1;
            r.showTileCell = false;
            r.tileRowspan = 1;
            r.showResourceCell = false;
            r.resourceRowspan = 1;
            r.showStatusCell = false;
            r.statusRowspan = 1;
        });
        ...
        for (let i = 0; i < slice.length; i++) {
            if (i === 0 || slice[i].groupKey !== slice[i - 1].groupKey) {
                ...
                while (
                    i + span < slice.length &&
                    slice[i + span].groupKey === slice[i].groupKey
                )
                    span++;
        ...
        for (let i = 0; i < slice.length; i++) {
            if (i === 0 || slice[i].tileKey !== slice[i - 1].tileKey) {
                ...
                while (
                    i + span < slice.length &&
                    slice[i + span].tileKey === slice[i].tileKey
                )
                    span++;
    ...
    this.displayInformation = function () {
        ...
        self.addAllFormData();
        self.submit("get_mergeable_nodegroups")
            .then((data) => {
                ...
                if (data.result.info == "Yes") {
                    ...
                    self.previewSortField(null);
                    self.previewSortDirection("asc");
                    const normalizedNodeGroups = normalizeMergeableGroups(
                        data.result.data,
                    );
                    this.mergeableNodeGroups(normalizedNodeGroups);
                    this.filteredMergeableNodeGroups(
                        self.applyExclusions(normalizedNodeGroups),
                    );
                    this.currentPage(1);
                    // dict_counts table
    ...
    this.recalculateDictCounts = function () {
        ...
        self.submit("recalculate_dict")
            .then(function (data) {
                console.log(
                    "Received data from backend (recalcluate_dict):",
                    data,
                );
    ...
    this.submit = function (action) {
        ...
        self.formData.append(
            "excluded_tile_ids",
            JSON.stringify(Array.from(self.excludedTileIds())),
        );
        return $.ajax({
            type: "POST",
    ...
    this.filterFinalData = () => {
        ...
        const filtered = this.finalDataGroups().filter(
            (group) =>
    ...
    this.userNameMerged = ko.observable(this.load_details?.userNameMerge || "");
    this.exportChangeSummaryCSV = () => {
        ...
        // ===================================================
        csv +=
            [escapeCSV("User"), escapeCSV(self.userNameMerged() || "—")].join(
                ",",
            ) + "\n\n";
...
export default ko.components.register("Resources_merge", {
  viewModel: ViewModel,
